...
var slideController = {
    ...
    registerEvent: function () {
        ...
        $('.btn-Choose').off('click').on('click', function (e) {
            e.preventDefault();
            var id = $(this).data('id');
            var target = $(this);
            $.ajax({
                url: '/admin/slide/ChooseImage',
                data: { id: id },
                type: 'post',
                dataType: 'json',
                success: function (res) {
        ...
        $.ajax({
            ...
            success: function (res) {
                if (res.status === true) {
                    ...
                    $('#EditThumbnail').attr('src', item.image);
                    tinymce.get('EditContent').setContent(item.content);
                    $('#EditDisplayOrder').val(item.displayOrder);
                    $('#EditAuthor').val(item.authors);
                    $('#EditChoose').prop('checked', item.isChoose);
    ...
    Post: function () {
        ...
        var IsChoose = $('#txtChoose').prop('checked');
        var e = {
            Name: Name,
            Image: Image,
            Content: Content,
    ...
    Put: function () {
        ...
        var IsChoose = $('#EditChoose').prop('checked');
        var e = {
            Id: id,
            Name: Name,
            Image: Image,
                ...
                if (response.status) {
                    ...
                    $.each(data, function (i, item) {
                        html += Mustache.render(template, {
                            ...
                            DisplayImage: item.isChoose ? " <a href='#' class='btn-Choose label-success' data-id='" + item.id + "'><i class='fa fa-check'></i><\/a>" : "<a href='#' class='btn-Choose label-danger' data-id='" + item.id + "'><i class='fa fa-ban'></i><\/a>"
                        });
                    });
                    $('#tbData').html(html);
